...
/* ------------------------------------------------------------------ */
/* Datos de ejemplo (solo para esta DEMO — no hay base de datos real)  */
/* ------------------------------------------------------------------ */
...
function buildDemoTenants() {
  ...
  const paidThisAndLastMonth = () => {
    const pagos = {};
    pagos[ymKey(y, m)] = true;
    let pm = m - 1, py = y;
    if (pm < 1) { pm = 12; py -= 1; }
    pagos[ymKey(py, pm)] = true;
    return pagos;
  };

  return [
    {
      id: uid(), habitacion: "A1", nombre: "Laura", apellidos: "Martín Ruiz",
      tipoDocumento: "DNI", numeroDocumento: "12345678A", nacionalidad: "Española",
      telefono: "600111222", correo: "laura.martin@example.com",
      fechaInicio: iso(y, m > 2 ? m - 2 : 12 + m - 2, 1), fechaFin: iso(y, m < 11 ? m + 2 : m + 2 - 12, 28),
      renovado: false, nuevaFechaFin: "", empadronado: true, renta: 420,
      fianzaImporte: 420, fechaPagoFianza: iso(y, m > 2 ? m - 2 : 12 + m - 2, 1), fechaDevolucionFianza: "",
      observaciones: "Inquilina de ejemplo. Paga siempre puntual.", activo: true, pagos: paidThisAndLastMonth()
    },
    {
      id: uid(), habitacion: "A2", nombre: "Carlos", apellidos: "Fernández Gil",
      tipoDocumento: "NIE", numeroDocumento: "X1234567B", nacionalidad: "Italiana",
      telefono: "600222333", correo: "carlos.fg@example.com",
      fechaInicio: iso(y, m, 1), fechaFin: iso(y, m < 10 ? m + 3 : m + 3 - 12, 30),
      renovado: false, nuevaFechaFin: "", empadronado: true, renta: 395,
      fianzaImporte: 395, fechaPagoFianza: iso(y, m, 1), fechaDevolucionFianza: "",
      observaciones: "", activo: true, pagos: { [ymKey(y, m)]: false }
    },
    {
      id: uid(), habitacion: "B1", nombre: "Ana", apellidos: "Sánchez López",
      tipoDocumento: "DNI", numeroDocumento: "87654321C", nacionalidad: "Española",
      telefono: "600333444", correo: "ana.sanchez@example.com",
      fechaInicio: iso(y, m > 5 ? m - 5 : 12 + m - 5, 15), fechaFin: iso(y, m < 8 ? m + 4 : m + 4 - 12, 14),
      renovado: true, nuevaFechaFin: iso(y, m < 8 ? m + 4 : m + 4 - 12, 14), empadronado: false, renta: 450,
      fianzaImporte: 450, fechaPagoFianza: iso(y, m > 5 ? m - 5 : 12 + m - 5, 15), fechaDevolucionFianza: "",
      observaciones: "Contrato ya renovado una vez.", activo: true, pagos: paidThisAndLastMonth()
    },
    {
      id: uid(), habitacion: "E2", nombre: "Miguel", apellidos: "Torres Vega",
      tipoDocumento: "Pasaporte", numeroDocumento: "PA1122334", nacionalidad: "Colombiana",
      telefono: "600444555", correo: "miguel.torres@example.com",
      fechaInicio: iso(y, m > 3 ? m - 3 : 12 + m - 3, 1), fechaFin: iso(y, m, 20),
      renovado: false, nuevaFechaFin: "", empadronado: true, renta: 410,
      fianzaImporte: 410, fechaPagoFianza: iso(y, m > 3 ? m - 3 : 12 + m - 3, 1), fechaDevolucionFianza: "",
      observaciones: "Fin de contrato próximo — ejemplo de alerta.", activo: true, pagos: paidThisAndLastMonth()
    },
    {
      id: uid(), habitacion: "C1", nombre: "Sofía", apellidos: "Ibáñez Prat",
      tipoDocumento: "DNI", numeroDocumento: "11223344D", nacionalidad: "Española",
      telefono: "600555666", correo: "sofia.ibanez@example.com",
      fechaInicio: iso(y, m > 8 ? m - 8 : 12 + m - 8, 1), fechaFin: iso(y, m > 1 ? m - 1 : 12, 30),
      renovado: false, nuevaFechaFin: "", empadronado: true, renta: 430,
      fianzaImporte: 430, fechaPagoFianza: iso(y, m > 8 ? m - 8 : 12 + m - 8, 1), fechaDevolucionFianza: "",
      observaciones: "Ejemplo de inquilina en el histórico (habitación ya liberada).", activo: false, pagos: {}
    }
  ];
}
...
function buildDemoExpenses() {
  ...
  const obj = {};
  obj[ym] = { luz: 145, agua: 60, gas: 40, limpieza: 90, internet: 45, ibi: 0, comunidad: 120, reparaciones: 35, otros: 20, concepto: "Bombilla y material de limpieza extra" };
  return obj;
}
...
const GlobalStyles = () => (
  <style>{`
    ...
    .rg-demo-badge {
      ...
      letter-spacing: .03em; color: var(--accent); background: var(--accent-dim);
      border-radius: 7px; padding: 6px 10px; margin: 0 4px 16px 4px;
    }
    .rg-demo-banner {
      display: flex; align-items: center; gap: 10px; background: var(--accent-dim); color: var(--text);
      border: 1px solid var(--accent); border-radius: 10px; padding: 10px 16px; font-size: 12.5px;
      margin-bottom: 20px; line-height: 1.5;
    }
    .rg-demo-banner svg { color: var(--accent); flex-shrink: 0; }
    ...
    <div className="rg-root">
      ...
      <div className="rg-shell">
        ...
        <div className="rg-sidebar">
          ...
          <div className="rg-demo-badge">
            <Sparkles size={13} /> MODO DEMO — los datos no se guardan
          </div>
          ...
          <div className="rg-sidebar-footer">
            <span className="rg-save-indicator"><Sparkles size={12} color="var(--accent)" /> Datos de ejemplo, en memoria</span>
            <button className="rg-icon-btn" style={{ marginTop: 4 }} onClick={resetDemo} title="Reiniciar demo">
              <RotateCcw size={12} /> <span style={{ fontSize: 11 }}>Reiniciar demo</span>
            </button>
          </div>
        </div>
        ...
        <div className="rg-main">
          <div className="rg-demo-banner">
            <Sparkles size={14} />
            Estás viendo una <strong>demo</strong> de la aplicación con datos de ejemplo. Nada de lo que cambies aquí se guarda de forma permanente: al recargar la página, todo vuelve al punto de partida.
          </div>
          {view === "dashboard" && (
            <DashboardView
